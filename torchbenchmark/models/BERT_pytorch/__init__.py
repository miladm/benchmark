...
class Model(BenchmarkModel):
        ...
        if test == "train":
            ...
            if args.script:
                if hasattr(torch.jit, '_script_pdt'):
                    self.model = torch.jit._script_pdt(bert, example_inputs=[self.example_inputs, ])
                else:
            ...
            if args.script:
                if hasattr(torch.jit, '_script_pdt'):
                    self.eval_model = torch.jit._script_pdt(bert, example_inputs=[self.eval_example_inputs, ])
                else:
    ...
    def eval(self, niter=1):
        ...
        for _ in range(niter):
            ...
            next_sent_output, mask_lm_output = eval_model.model.forward(*self.example_inputs)
